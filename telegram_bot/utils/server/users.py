...
class UserCommandState:
    ...
    def _run_query(self):
        query = db.execute("SELECT last_command,require_follow_up FROM usercommandstate WHERE user_id = :user_id AND chat_id = :chat_id", {"user_id": self.user_id, "chat_id": self.chat_id})
